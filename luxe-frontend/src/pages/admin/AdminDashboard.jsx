  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
    ...
    <motion.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button onClick={onConfirm} disabled={loading} className="flex-1 bg-red-500/20 border border-red-500/50 text-red-400 hover:bg-red-500/30 py-2.5 rounded-xl transition text-sm disabled:opacity-50">
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={add} className="flex items-center gap-1 text-xs text-gold hover:text-yellow-300 transition border border-gold/30 px-2 py-1 rounded-lg hover:bg-gold/10">
...
>>>>>>> 5d434ed (feat: admin user profile, multi-address, avatar upload, email change OTP, checkout modal)
      ...
      {displayUrls.map((url, i) => (
        <div key={i} className="flex gap-2 items-center">
          <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-800 flex-shrink-0 border border-gray-700">
            {url ? <img src={url} alt="" className="w-full h-full object-cover" onError={e => { e.target.style.display = "none"; }} /> : <div className="w-full h-full flex items-center justify-center text-gray-600"><ImgIcon /></div>}
          </div>
...
>>>>>>> 5d434ed (feat: admin user profile, multi-address, avatar upload, email change OTP, checkout modal)
  ...
  const selectedLabel = value ? categoryOptions.find(c => String(c.id) === String(value))?.name || "Select category" : "No Category";
      ...
      <div className="flex items-center gap-2">
        <div ref={dropdownRef} className="flex-1 relative">
          <button type="button" onClick={() => setDropdownOpen(v => !v)}
            className={`w-full flex items-center justify-between bg-transparent border p-3 rounded-lg text-sm transition focus:outline-none ${dropdownOpen ? "border-gold text-white" : "border-gray-600 text-white hover:border-gray-500"}`}>
...
>>>>>>> 5d434ed (feat: admin user profile, multi-address, avatar upload, email change OTP, checkout modal)
                ...
                {allOptions.map(c => {
                  ...
                  return (
                    <li key={c.id}>
                      <button type="button" onClick={() => { onChange(String(c.id)); setDropdownOpen(false); }}
        ...
        {showInlineAdd && (
          <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
            ...
            <input type="text" placeholder="Category name" value={newName}
              onChange={e => { setNewName(e.target.value); setNewSlug(e.target.value.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "")); setErr(""); }}
  ...
  const SidebarContent = () => (
    <>
      <div className="p-6 border-b border-gold/10 flex items-center justify-between">
        ...
        <button className="lg:hidden text-gray-400 hover:text-white transition" onClick={() => setSidebarOpen(false)}><CloseIcon /></button>
      </div>
...
>>>>>>> 5d434ed (feat: admin user profile, multi-address, avatar upload, email change OTP, checkout modal)
      ...
      <div className="p-4 border-t border-gold/10">
        ...
        <button onClick={handleLogout} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-400 hover:text-red-400 transition rounded-lg hover:bg-red-400/5">
          <LogoutIcon /> Logout
        </button>
      </div>
    </>
...
>>>>>>> 5d434ed (feat: admin user profile, multi-address, avatar upload, email change OTP, checkout modal)
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="lg:hidden flex items-center justify-between px-4 py-3 glass border-b border-gold/10 sticky top-0 z-30">
          <button onClick={() => setSidebarOpen(true)} className="p-2 rounded-lg text-gray-400 hover:text-gold hover:bg-gold/10 transition"><MenuIcon /></button>
                  ...
                  <PieChart>
                    <Pie data={statusData} cx="50%" cy="50%" innerRadius="45%" outerRadius="70%"
                      ...
                      {statusData.map((entry, i) => (
                        <Cell key={entry.name} fill={STATUS_COLORS[entry.name] || RCOLORS[i % RCOLORS.length]} stroke="transparent" />
                      ))}
                    </Pie>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-700/50 text-gray-400 text-xs uppercase tracking-wide">
                <th className="text-left px-5 py-3">User</th><th className="text-left px-5 py-3">Email</th>
                <th className="text-left px-5 py-3">Role</th><th className="text-left px-5 py-3">Status</th>
                <th className="text-left px-5 py-3">Joined</th><th className="text-left px-5 py-3">Actions</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {!u.is_active && u.ban_reason && <p className="text-red-400 text-xs mt-0.5 truncate max-w-[140px]" title={u.ban_reason}>Reason: {u.ban_reason}</p>}
                      </div>
                    </div>
                  </td>
...
>>>>>>> 5d434ed (feat: admin user profile, multi-address, avatar upload, email change OTP, checkout modal)
                      ...
                      <button onClick={() => setDeleteTarget(u)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-400 hover:bg-red-400/10 transition"><TrashIcon /></button>
                    </div>
                  </td>
                </tr>
              ))}
...
>>>>>>> 5d434ed (feat: admin user profile, multi-address, avatar upload, email change OTP, checkout modal)
  ...
  const filteredProducts = products.filter(p => {
    ...
    const matchesCategory = !filterCategory || String(p.category?.id ?? p.category ?? "") === filterCategory;
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {search.trim() && (
                <span className="flex items-center gap-1 text-xs bg-gold/10 border border-gold/25 text-yellow-400 px-2.5 py-1 rounded-full">
                  "{search.trim()}" <button onClick={() => setSearch("")} className="ml-0.5 hover:text-white transition"><XIcon /></button>
                </span>
              )}
              {filterCategory && (
                <span className="flex items-center gap-1 text-xs bg-gold/10 border border-gold/25 text-yellow-400 px-2.5 py-1 rounded-full">
                  {selectedCatLabel} <button onClick={() => setFilterCategory("")} className="ml-0.5 hover:text-white transition"><XIcon /></button>
                </span>
              )}
...
>>>>>>> 5d434ed (feat: admin user profile, multi-address, avatar upload, email change OTP, checkout modal)
        ...
        {deleteTarget && <DeleteConfirmModal message={`Remove "${deleteTarget.name}"?`} subMessage="This product will be permanently deleted." onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} loading={deleteLoading} />}
      </AnimatePresence>
    </motion.div>
  );
}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      {loading ? <p className="text-gray-400">Loading...</p> : (
        <>
          <div className="glass rounded-2xl overflow-hidden hidden sm:block">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-gray-700/50 text-gray-400 text-xs uppercase tracking-wide">
                  <th className="text-left px-5 py-3">Name</th><th className="text-left px-5 py-3">Slug</th><th className="text-left px-5 py-3">Actions</th>
                </tr></thead>
                <tbody>
                  ...
                  {categories.map(c => (
                    <tr key={c.id} className="border-b border-gray-800/50 hover:bg-white/2 transition">
                      <td className="px-5 py-4"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-gold/10 border border-gold/20 flex items-center justify-center text-gold flex-shrink-0"><TagIcon /></div><span className="text-white font-medium">{c.name}</span></div></td>
          ...
          <div className="sm:hidden space-y-3">
            ...
            {categories.map(c => (
              <div key={c.id} className="glass rounded-xl p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0"><p className="text-white font-medium">{c.name}</p><p className="font-mono text-xs text-gray-400 mt-0.5">{c.slug}</p></div>
      ...
      <AnimatePresence>
        ...
        {deleteTarget && <DeleteConfirmModal message={`Delete "${deleteTarget.name}"?`} subMessage="Products in this category will lose their category assignment." onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} loading={deleteLoading} />}
      </AnimatePresence>
    </motion.div>
  );
}
