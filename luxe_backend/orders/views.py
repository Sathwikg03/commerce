...
class AdminOrderListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if search := self.request.query_params.get('search'): qs = qs.filter(Q(user__username__icontains=search) | Q(user__email__icontains=search))
        if f      := self.request.query_params.get('from'):   qs = qs.filter(created_at__date__gte=f)
        if t      := self.request.query_params.get('to'):     qs = qs.filter(created_at__date__lte=t)
        return qs
